...
[ ! -d wt-${WTV}-src-http ] && echo "No Project: wt-${WTV}-src-http" && exit -1
sed -i \
    ...
    wt-${WTV}-src-http/CMakeLists.txt
mkdir -p wt-${WTV}-src-http/build/{CCMake-Debug,CCMake-MinSizeRel,CCMake-Profile,CCMake-Release,CCMake-RelWithDebInfo}
ccmake -B wt-${WTV}-src-http/build/CCMake-Debug -S wt-${WTV}-src-http \
