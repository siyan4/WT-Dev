...
[ ! -d wt-${WTV}-jbc-http ] && echo "No Project: wt-${WTV}-jbc-http" && exit -1
sed -i \
    ...
    wt-${WTV}-jbc-http/CMakeLists.txt
mkdir -p wt-${WTV}-jbc-http/build/{CCMake-Debug,CCMake-MinSizeRel,CCMake-Profile,CCMake-Release,CCMake-RelWithDebInfo}
ccmake -B wt-${WTV}-jbc-http/build/CCMake-Debug -S wt-${WTV}-jbc-http \
