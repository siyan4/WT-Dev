...
[ ! -d wt-${WTV}-ccm-http ] && echo "No Project: wt-${WTV}-ccm-http" && exit -1
sed -i \
    ...
    wt-${WTV}-ccm-http/CMakeLists.txt
mkdir -p wt-${WTV}-ccm-http/build/{CCMake-Debug,CCMake-MinSizeRel,CCMake-Profile,CCMake-Release,CCMake-RelWithDebInfo}
ccmake -B wt-${WTV}-ccm-http/build/CCMake-Debug -S wt-${WTV}-ccm-http \
