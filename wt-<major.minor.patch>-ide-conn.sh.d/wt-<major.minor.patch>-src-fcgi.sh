...
[ ! -d wt-${WTV}-src-fcgi ] && echo "No Project: wt-${WTV}-src-fcgi" && exit -1
sed -i \
    ...
    wt-${WTV}-src-fcgi/CMakeLists.txt
mkdir -p wt-${WTV}-src-fcgi/build/{CCMake-Debug,CCMake-MinSizeRel,CCMake-Profile,CCMake-Release,CCMake-RelWithDebInfo}
ccmake -B wt-${WTV}-src-fcgi/build/CCMake-Debug -S wt-${WTV}-src-fcgi \
	-D CMAKE_BUILD_TYPE=Debug -D BUILD_FUZZ=OFF -D DEBUG=ON -D ENABLE_SAML=ON -D ENABLE_UNWIND=ON \
	-D EXAMPLES_CONNECTOR=wtfcgi \
	-D IBPP_SRC_DIRECTORY=/home/sib/Projects/ibpp.d/core -D INSTALL_DOCUMENTATION=ON -D INSTALL_EXAMPLES=ON -D UNWIND_PREFIX=/usr \
	-D WEBGROUP=www-data -D WEBUSER=www-data \
	-D WT_CPP20_DATE_TZ_IMPLEMENTATION=std -D WT_WRASTERIMAGE_IMPLEMENTATION=GraphicsMagick
#
#EOS
